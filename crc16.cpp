...
//unsigned short crc16_ccitt(char buf, int len)
{
	...
	for(counter = 0; counter<len; counter++){
		...
		//crc = (crc<<8)^crc16tab[((crc>>8) ^ 0x006E)&0x00FF];			//8D68
		//crc = (crc<<8)^crc16tab[((crc>>8) ^ (char)0x6E)&0x00FF];		//8D68
		//crc = (crc<<8)^crc16tab[((crc>>8) ^ 0x6E)&0x00FF];			//8D68
	}
	return crc;
}
